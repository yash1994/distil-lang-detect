...
@click.option(
    ...
    required=True,
    type=str,
    help="Name of model you want to use from: {}".format(
        list(model_config.get_model_list())
    ),
)
def download(model_name):
    if model_config.check_model_presence(model_name):
        ...
        if os.path.exists(distillangdetect_model_dir):
            (
                integrity,
                distillangdetect_model_dir,
            ) = model_config.get_model_path_and_varify_integrity(model_name)
            if not integrity:
                print(
                    "Model path already exists, but files verification failed, removing directory and reinstalling model."
                )
                shutil.rmtree(distillangdetect_model_dir)
            else:
                print(
                    "Model {} already exists at {}, integrity varified.".format(
                        model_name, distillangdetect_model_dir
                    )
                )
                return
        ...
        os.system(
            "wget -O {}.zip {}".format(distillangdetect_model_dir, model_dropbx_link)
        )
        ...
        os.system(
            "unzip {}.zip -d {}/".format(
                distillangdetect_model_dir, distillangdetect_model_dir
            )
        )
        ...
        print(
            "Model name you've specified is not available. Specify one from {}".format(
                model_config.get_model_list()
            )
        )
